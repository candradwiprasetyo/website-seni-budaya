...
function insert_favourite($data){
    $CI =& get_instance();
    $CI->db->insert('favourite', $data);
}

 function delete_favourite($type, $document_id)
    {
        $CI =& get_instance();
        $CI->db->where('favourite_type', $type);  
        $CI->db->where('favourite_document_id', $document_id);  
        $CI->db->delete('favourite');
    }

 function update_favourite($id)
    {
        $ci =& get_instance();
        $data = array(
            "favourite_folder" => '0',
        );
        $ci->db->where('favourite_id', $id);
        $ci->db->update('favourite', $data);
    }
...
function get_data_banner() {
    ...
    $ci->db->order_by(' ordernum ASC ');
    return $ci->db->get()->result();
}

function get_data_content($type) {

    if ($type == 'frequently-asked-question') {

        $ci =& get_instance();
        $ci->db->select('faq_isi as page_content');
        $ci->db->from('faq');
        return $ci->db->get()->result();
    } else {
        $ci =& get_instance();
        $ci->db->select('disclaimer_isi as page_content');
        $ci->db->from('disclaimer');
        return $ci->db->get()->result();
    }
}

function check_favourite($type, $document_id) {
    $CI =& get_instance();
    $sql = "select count(favourite_id) as result from favourite where favourite_type = '$type' and favourite_document_id = '$document_id'";    
...
function get_data_favourite_folder($id) {
    $ci =& get_instance();
    $ci->db->select('*');
    $ci->db->from('favourite');
    $ci->db->where("favourite_folder='$id'");
    $ci->db->order_by(' favourite_id ASC ');
    return $ci->db->get()->result();
}
